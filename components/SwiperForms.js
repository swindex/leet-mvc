 ...
 */
export class SwiperForms extends BaseComponent{
	...
	constructor(formTemplate, data, errors, options){
		...
		if (Objects.filter(formTemplate,(el)=>{return el.type =='form' }).length>1){
		...
		<div class="swiper-container" id="generatedform">
			<div class="swiper-wrapper">
				`+this.render(this.formTemplate, null)+`
			</div>
	...
	swiperElementValidatePhone(evt){
		var el  = evt.target;
	...
	/*self.validator.validateVisibility();
	self.events = {
		change:(ev)=>{
			console.log(ev);
			//Inject.Storage.set('RegisterPage',{data: this.data});
			if (ev.target.name){
				self.validator.validateField(ev.target.name);
				self.binder.updateElements();
			}
		}
	}*/
	...
	render_field(el,formName){
		switch (el.type){
			...
			case "phone":
				return this.addInput(el,{type:'tel', oninput:"this.swiperElementValidatePhone($event)"},formName);
			case "checkbox":
	...
	addForm(el, override){
		...
		return `
		<div class="swiper-slide formgroup scroll">
			<h1>${el.title}</h1>
			<div>
			${elem}
			</div>
	...
	addSelect(el, override,formName){
		...
		var opt = { name: el._name, type: "text" };
		$.extend(opt, override);
		var elem = $('<select />',opt).attr('bind',`this.data.${el._name}`);
		if (el.placeholder)
			elem.append($('<option/>', {text:el.placeholder, value:''}));
		$.each(el.items, function (index, option) {
			//if (el.value == option.value){
			//	option['selected'] = true;
				//elem.find('option').first().prop('selected',null);
			//}
