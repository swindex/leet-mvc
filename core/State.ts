...
export interface StateListener<T, E = any> {
  index: number;
  destroy: () => void;
  remove: () => void;
  onSet: (callback: (data: T) => void) => StateListener<T, E>;
  onError: (callback: (error: E) => void) => StateListener<T, E>;
  onStatus: (callback: (isRunning: boolean) => void) => StateListener<T, E>;
}

export interface StateInstance<T, E = any> {
  newListener: () => StateListener<T, E>;
  error: E;
  isRunning: boolean;
  data: T;
  destroy: () => void;
  remove: (listener: StateListener<T, E>) => boolean;
 ...
 */
export function State<T, E = any>(data: T): StateInstance<T, E> {
   ...
   */
  function add(): StateListener<T, E> {
    index++;
    Queue[index] = {};

    const i = index;
    const listener: StateListener<T, E> = {
      index: i,
      destroy: function () {
        removeIndex(i);
   ...
   */
  function remove(listener: StateListener<T, E>): boolean {
